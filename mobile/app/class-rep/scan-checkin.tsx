...
import { checkInStudent } from '../../src/api/class-rep';
...
// — same URL shape app/scan.tsx and the web class-rep scanner already
// understand, so the one QR code works everywhere.
...
// Class-rep-only variant of app/scan.tsx — scoped to marking a student
// present in a specific open attendance session (not self-check-in, not
// manual QR verification, not Connect). Reached from the "Scan Student QR"
// action on the class-rep attendance screen once a session is open.
export default function ClassRepScanCheckinScreen() {
  ...
  const handleScan = async (scan: BarcodeScanningResult) => {
    if (locked || scan.data === lastScan.current || !sessionId) return;
    ...
    if (!scannedUserId) {
      haptics.error();
      setResult({ ok: false, text: 'This does not look like a student ID QR code.' });
      return;
    }

    try {
      const checkin = await checkInStudent(sessionId, scannedUserId, 'qr', true);
      haptics.success();
      setResult({ ok: true, text: `${checkin.student_name || 'Student'} marked present.` });
    } catch (err) {
      haptics.error();
      setResult({ ok: false, text: getErrorMessage(err, 'Could not check in this student.') });
    }
  };
    ...
    return (
      <View style={[styles.flex, styles.center, { backgroundColor: theme.background, padding: spacing.xl }]}>
        ...
        <Text style={[styles.permBody, { color: theme.textMuted }]}>
          Scan a student's profile QR code to mark them present.
        </Text>
  ...
  return (
    <View style={styles.flex}>
      ...
      <View style={[styles.overlay, { paddingTop: insets.top + spacing.md }]}>
        ...
        <Text style={styles.overlaySubtitle}>Scan a student's profile QR code to mark them present</Text>
      ...
      {result && (
        <Animated.View entering={FadeIn.duration(200)} style={styles.resultBackdrop}>
          ...
          >
            ...
            <Text style={[styles.resultTitle, { color: theme.text }]}>{result.ok ? 'Checked In' : 'Scan Failed'}</Text>
            <Text style={[styles.resultBody, { color: theme.textMuted }]}>{result.text}</Text>
            <View style={styles.resultActions}>
              <View style={styles.flex}>
                <Button label="Scan Next" variant="outline" onPress={reset} fullWidth />
              </View>
              <View style={styles.flex}>
