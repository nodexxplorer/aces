...
export default function QRScanPage() {
  ...
  const handleScan = async (data: string) => {
    setScanning(false);
    try {
      const result = await verifyManualQR(data);
      ...
      const msg = err?.response?.data?.error || err?.message || 'Invalid QR code';
  ...
  return (
    <div className="space-y-6 max-w-lg mx-auto">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="p-2 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors">
      ...
      <Card>
        ...
        <div className="p-4 pt-0 space-y-4">
          {!scanning ? (
            <Button onClick={() => { setScanning(true); setLastResult(null); }} leftIcon={<ScanLine className="w-4 h-4" />}>
              Start Scanning
            </Button>
          ) : (
      ...
      {lastResult && (
        <Card>
          <div className={`p-4 flex items-start gap-3 ${lastResult.success ? 'bg-success-50 dark:bg-success-900/10' : 'bg-danger-50 dark:bg-danger-900/10'} rounded-xl`}>
            {lastResult.success ? (
            ...
            <div>
              <p className={`text-sm font-medium ${lastResult.success ? 'text-success-700 dark:text-success-400' : 'text-danger-700 dark:text-danger-400'}`}>
