...
export default function ModerationPage() {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const data = await listCampusReports(params);
      setReports(data);
    } catch {
  ...
  const handleReview = async (report: Report) => {
    ...
    try {
      await updateCampusReportStatus(report.id, { status: 'reviewed' });
      notify('Report marked as reviewed', 'success');
      fetchReports();
    } catch {
  ...
  const handleResolve = async () => {
    ...
    try {
      await updateCampusReportStatus(resolveModal.id, {
        status: 'resolved',
  ...
  const handleDismiss = async (report: Report) => {
    setActionLoading(report.id);
    try {
      await updateCampusReportStatus(report.id, { status: 'dismissed' });
  ...
  const handleIssueStrike = async () => {
    ...
    try {
      await issueStrike({ target_user_id: strikeTarget, reason: strikeReason });
      notify('Strike issued successfully', 'success');
      setStrikeModal(false);
      setStrikeTarget('');
      setStrikeReason('');
    } catch {
      notify('Failed to issue strike', 'error');
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button onClick={() => setStrikeModal(true)} variant="destructive" className="flex items-center gap-2">
