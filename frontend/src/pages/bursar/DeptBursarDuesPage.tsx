...
const DeptBursarDuesPage = () => {
  ...
  const fetchDues = async () => {
    try {
      ...
      setDues(list.filter((d) => d.type === 'dept_dues'));
    } catch {
      notifyError('Error', 'Failed to load department dues');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-surface-900 dark:text-white">Department Dues</h1>
          <p className="text-sm text-surface-500 dark:text-surface-400 mt-1">
            Manage department association dues — create, view, and deactivate.
          </p>
        </div>
        <button
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>New Department Due</CardTitle>
          </CardHeader>
          <form onSubmit={handleCreate} className="p-4 pt-0 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600" value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Department Dues 2025/2026" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">Level (optional)</label>
                <select className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600" value={level} onChange={(e) => setLevel(e.target.value)}>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Department Dues Registry</CardTitle>
