...
// The course-results endpoint returns raw DB rows (snake_case), which don't
// always line up with the frontend `Result` type's camelCase fields.
...
const EnterScoresPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(
        ((res || []) as unknown as CourseResultRow[]).map((r) => ({
          resultId: r.id,
  ...
  const handleSave = async () => {
    for (const r of rows) {
      ...
      if (ca > 30 || exam > 70) {
        warning('Input Error', `CA ≤ 30, Exam ≤ 70. Check ${r.name}.`);
        return;
      }
    }
    setSaving(true);
    ...
    for (const r of rows) {
      ...
      try {
        await updateScore(r.resultId, { caScore: parseFloat(r.ca || '0'), examScore: parseFloat(r.exam || '0') });
        ok++;
      } catch {
        fail++;
      }
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between gap-4">
          <div>
            ...
            <CardDescription>Edit scores for the selected course</CardDescription>
          </div>
          {loadingA ? (
            <Loader2 className="w-5 h-5 animate-spin text-primary-500" />
          ) : (
            <Select
              options={assignments.map((a) => ({ value: a.id, label: `${a.course_code} — ${a.course_title}` }))}
              value={selectedId}
                    ...
                    <tr key={i} className={isErr ? 'bg-danger-500/5' : ''}>
                      ...
                      <td className="px-6 py-2">
                        <input
                          ...
                          value={r.ca}
                          maxLength={4}
                          onChange={(e) => change(i, 'ca', e.target.value)}
                          className={`w-20 px-2 py-1 text-sm bg-white dark:bg-surface-900 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500/20 ${ca > 30 ? 'border-danger-500' : 'border-surface-300 dark:border-surface-600'}`}
                        />
                      </td>
                      <td className="px-6 py-2">
                        <input
                          ...
                          value={r.exam}
                          maxLength={4}
                          onChange={(e) => change(i, 'exam', e.target.value)}
                          className={`w-20 px-2 py-1 text-sm bg-white dark:bg-surface-900 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500/20 ${exam > 70 ? 'border-danger-500' : 'border-surface-300 dark:border-surface-600'}`}
                        />
                      </td>
                      <td className="px-6 py-4 font-bold">{isErr ? '-' : total}</td>
                      <td className="px-6 py-4 font-bold text-primary-500">
                        {isErr ? <AlertCircle className="w-5 h-5 text-danger-500" /> : calculateGrade(total)}
                      </td>
                    </tr>
                  );
                })}
