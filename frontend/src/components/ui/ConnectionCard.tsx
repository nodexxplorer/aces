  ...
  <Card hover className="flex flex-col" onClick={onViewProfile}>
    <div className="flex items-start gap-3 mb-3">
      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary-400 to-accent-500 flex items-center justify-center text-white font-semibold text-sm shrink-0">
        {user.avatar ? (
          <img src={getMediaUrl(user.avatar) ?? undefined} alt="" className="w-full h-full rounded-full object-cover" />
        ) : (
    ...
    <div className="flex items-center gap-2 mt-auto pt-3 border-t border-surface-100 dark:border-surface-700">
      {connectionStatus === 'none' && onConnect && (
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 px-2.5 py-1.5 rounded-lg transition-colors font-medium"
        >
      ...
      <button className="ml-auto p-1.5 text-surface-400 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-lg">
