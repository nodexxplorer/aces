...
func (q *Queries) GetPaymentByReference(ctx context.Context, paystackReference *string) (GetPaymentByReferenceRow, error) {
...
const listDefaultersByLevel = `-- name: ListDefaultersByLevel :many
SELECT
    s.id AS student_id,
    u.full_name,
    s.matric_number,
...
func (q *Queries) ListDefaultersByLevel(ctx context.Context) ([]ListDefaultersByLevelRow, error) {
	rows, err := q.db.Query(ctx, listDefaultersByLevel)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListDefaultersByLevelRow{}
	for rows.Next() {
		var i ListDefaultersByLevelRow
		if err := rows.Scan(
			&i.StudentID,
			&i.FullName,
			&i.MatricNumber,
